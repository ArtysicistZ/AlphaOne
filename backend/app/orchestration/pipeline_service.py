...
def run_pipeline_once(
    ...
    logger.info(
        "pipeline_run_started subreddits=%s fetch_limit=%s",
        len(subreddits),
        fetch_limit,
    )
    if not subreddits:
    ...
    try:
        ...
        from app.processing.sentiment_processing_service import process_all

        process_stats = process_all()
        result = {
...
def run_pipeline(subreddits: list[str], fetch_limit: int) -> dict:
    return run_pipeline_once(subreddits, fetch_limit)
